...
const { TherapistCalendar } = require('../models/Thcalendar');
...
const getAppointments = (req, res) => {
	Calendar.findOne({ UID: req.user.id })
		...
		.catch((err) => {
			res.status(400).json({
				message: `Error occurred while searching for user\'s calendar : ${err}`,
				success: false,
			});
		});
};
...
const createAppointment = (req, res) => {
	...
	if (!isValid) {
		...
		Calendar.findOne({ UID: req.user.id })
			.then((dbCalendar) => {
				...
				if (dbCalendar) {
					...
					dbCalendar
						...
						.catch((err) =>
							res.status(400).json({
								message: `Error occurred while saving event to db : ${err}`,
								success: false,
							})
						);
				} else {
					...
					newCal
						...
						.then(() =>
							res.status(200).json({
								message: 'Calendar initialized, Appointment created successfully',
								success: true,
							})
						)
						.catch((err) =>
			...
			.catch((err) => {
				res.status(400).json({
					message: `Error occurred while searching user\'s calendar : ${err}`,
					success: false,
				});
			});
	}
...
const modifyAppointment = (req, res) => {
	...
	if (!isValid) {
		...
		res.status(400).json({
			message: 'eventID provided is not a valid mongoDB objectid',
			success: false,
		});
	} else {
		Calendar.findOne({ UID: req.user.id })
			.then((dbCalendar) => {
				...
				if (appointmentFound) {
					...
					res.status(400).json({
						message: `No appointment with id ${req.params.eventID}`,
						success: false,
					});
				}
			})
...
const getAppointmentsTherapist = (req, res) => {
	TherapistCalendar.findOne({ therapistID: req.params.therapistID })
		.then((dbCalendar) => {
			if (dbCalendar) {
				res.status(201).json({ calendarEvents: dbCalendar.events, success: true });
			} else {
		...
		.catch((err) => {
			res.status(400).json({
				message: `Error occurred while searching for therapist\'s calendar : ${err}`,
				success: false,
			});
		});
};
...
const createAppointmentTherapist = (req, res) => {
	...
	if (!isValid) {
		...
		TherapistCalendar.findOne({ therapistID: req.body.id })
			.then((dbCalendar) => {
				const { title, description, date, repeat } = req.body;
				const newAppointment = { title, description, date, repeat };
				if (dbCalendar) {
					...
					let events = [];
					events.push(newAppointment);
					const newCal = new TherapistCalendar({
						therapistID: req.body.id,
						events,
					});
					newCal
						.save()
...
const modifyAppointmentTherapist = (req, res) => {
	const { title, description, date, repeat } = req.body;
	const { errors, isValid } = validateAppInput(req.body);
...
const deleteAppointmentTherapist = (req, res) => {
	...
	if (!validateObjectID(req.params.eventID)) {
		...
		TherapistCalendar.findOne({ therapistID: req.params.therapistID })
			...
			.catch((err) => {
				res.status(400).json({
					message: `Error occurred while searching therapist\'s calendar : ${err}`,
					success: false,
				});
			});
	}
...
module.exports = {
	...
	modifyAppointmentTherapist,
	deleteAppointmentTherapist,
};
