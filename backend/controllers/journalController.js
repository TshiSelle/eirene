...
const deleteJournal = async (req, res) => {
	...
	if (!isValid) {
		...
		Journal.deleteOne({ _id: journalID, _UID: req.user.id })
			...
			.catch((err) => {
				res.status(400).json({
					message: `Error occurred while searching for journal : ${err}`,
					success: false,
				});
			});
	}
...
const getJournals = (req, res) => {
	Journal.find({ _UID: req.user.id }, { __v: 0, _UID: 0 })
		...
		.catch((err) => {
			res.status(400).json({
				message: `Error occurred while searching for journals : ${err}`,
				success: false,
			});
		});
};
			...
			.then((dbJournal) => {
				if (dbJournal) {
					...
					dbJournal
						...
						.then(() => {
							res.status(201).json({
								message: 'Modified journal successfully',
								success: true,
							});
						})
						.catch((err) => {
					...
					res.status(400).json({
						message: `Journal of id ${journalID} is undefined for user ${req.user.id}`,
						success: false,
					});
				}
			})
